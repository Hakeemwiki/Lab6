...
if __name__ == "__main__":
    ...
    input_bucket = os.environ.get('S3_INPUT_BUCKET', 'lab6-ecommerce-shop')
    threshold = int(sys.argv[3])  # Threshold from command line
    ...
    s3_base_prefix = orders_path.split(input_bucket + '/')[1].split('*')[0].rstrip('/')
    orders_suffix = orders_path.split('*')[1] if '*' in orders_path else ''
    order_items_suffix = order_items_path.split('*')[1] if '*' in order_items_path else ''
    products_suffix = products_path.split('*')[1] if '*' in products_path else 'products.csv'
    
    # Validate files
    products_valid, products_count = validate_files(input_bucket, s3_base_prefix, products_suffix, 'products', 1)
    if not products_valid:
        logger.error("Products validation failed or threshold not met")
        sys.exit(1)
    
    orders_valid, orders_count = validate_files(input_bucket, s3_base_prefix, orders_suffix, 'orders', threshold)
    items_valid, items_count = validate_files(input_bucket, s3_base_prefix, order_items_suffix, 'order_items', threshold)
